...
Scrapes every EFP view XML and audits it for duplicate groups, tissues, and samples.

Reads:  each species' view XML at {efp_base}/data/{view_name}.xml
Writes: species_controls_samples.json — full nested view data (deduped)
        duplicate_log.json            — every duplicate found, with location context

Run this whenever a new EFP site is added or XML files are updated to catch
structural issues before they reach the API.
"""
...
def fetch_view_data(xml_url, db_name, view_name):
    """Fetch a view's XML, parse groups/controls/treatments, and log any duplicates.

    Duplicate groups, tissues, and samples are appended to the module-level
    ``duplicate_log`` list. Duplicate samples within a tissue are dropped from
    the returned data so the output remains usable even when issues exist.

    :param xml_url: Full URL to the view's XML file.
    :type xml_url: str
    :param db_name: Database name associated with this view.
    :type db_name: str
    :param view_name: Display name of the view (e.g. ``'Abiotic_Stress'``).
    :type view_name: str
        ...
        for group in root.findall(".//group"):
            ...
            if group_name in seen_group_names:
                duplicate_log.append(
                    {"xml": xml_url, "type": "group", "name": group_name}
                )
            ...
            for tissue in group.findall("tissue"):
                ...
                if t_key in seen_tissues:
                    duplicate_log.append(
                        {"xml": xml_url, "type": "tissue", "name": t_name, "group": group_name}
                    )
                ...
                for s in samples:
                    if s in seen_samples:
                        duplicate_log.append(
                            {"xml": xml_url, "type": "sample", "name": s, "tissue": t_name, "group": group_name}
                        )
                    else:
                ...
                treatments[t_key].extend(deduped_samples)
...
def main():
    """Iterate over all species and views, audit for duplicates, and write output files."""
    all_species_data = {}
    ...
    if duplicate_log:
        ...
        print("Duplicates detected! See duplicate_log.json")
    else:
